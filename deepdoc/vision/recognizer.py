...
class Recognizer:
    ...
    def sort_Y_firstly(arr, threshold):
        # Sort by Y coordinate (vertical position) primarily, X coordinate secondarily
        # Purpose: For document layout analysis where elements in the same "row"
        #          should be grouped together and sorted left-to-right within each row
        def cmp(c1, c2):
            # Step 1: Calculate Y coordinate difference
            # Principle: Primary sort by vertical position (top coordinate)
            diff = c1["top"] - c2["top"]

            # Step 2: Apply threshold-based row detection
            # Principle: If Y difference is small, consider elements on same row
            #            and sort by X coordinate (left-to-right) instead
            if abs(diff) < threshold:
                diff = c1["x0"] - c2["x0"]

            # Return value interpretation:
            #   diff < 0  -> c1 comes before c2 (c1 is above or left of c2)
            #   diff == 0 -> c1 and c2 have equal position
            #   diff > 0  -> c1 comes after c2 (c1 is below or right of c2)
            return diff

        # Use cmp_to_key adapter to use comparison function with Python's sorted()
        # Background: Python 3 removed the cmp parameter from sorted(), only key is supported
        # Solution: cmp_to_key creates a wrapper class that implements rich comparisons
        #           (__lt__, __gt__, etc.) using the provided comparison function
